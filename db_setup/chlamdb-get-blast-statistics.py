...
def get_best_non_top_phylum_hit(biodb):
    ...
    sql = """create table blastnr_blastnr_best_non_self_phylum (
                query_taxon_id INT,
                seqfeature_id INT,
                blast_hit_id INT,
    ...
    for taxon in taxon2most_freq_phylum:
        ...
        sql = 'select t1.id as blast_hit_id, t1.query_taxon_id, t1.seqfeature_id,t2.superkingdom from blastnr_blastnr t1 ' \
              ' inner join blastnr_blastnr_taxonomy t2 on t1.subject_taxid=t2.taxon_id ' \
              ' where (t1.query_taxon_id=%s and t2.phylum != "%s") order by seqfeature_id,hit_number' % (taxon,
                                                                                              phylum_filter)
        cursor.execute(sql)
        data = cursor.fetchall()
        df = pandas.read_sql(sql, engine)
        # keep best non top phylum hit for each feature
        df_BH = df.groupby("seqfeature_id").head(1)       

        print(df_BH.head())
        df_BH.to_sql("blastnr_blastnr_best_non_self_phylum", 
                                            engine, 
                                            index=False, 
                                            if_exists="append")
...
def best_blast_hit_majority_species(biodb):
    ...
    sql = 'CREATE table blastnr_blastnr_best_hits_species (taxon_id INT, hit_taxid INT, count INT, proportion FLOAT)'

    server.adaptor.execute(sql,)
    server.commit()

    sql = 'select taxon_id, count(*) as n from orthology_detail group by taxon_id'
    ...
    for one_taxon in taxon_list:
        ...
        for n, taxon in enumerate(taxon_id2count):
                sql = 'insert into blastnr_blastnr_best_hits_species values(%s,%s,%s, %s)' % (one_taxon,
                                                                                                 taxon,
...
def majority_phylum(biodb, n_hits):
    ...
    cursor.execute(sql)
    taxon_list = [i[0] for i in cursor.fetchall()]

    sql = 'CREATE table blastnr_blastnr_majority_phylum (taxon_id INT, seqfeature_id INT, best_hit_phylum varchar(400), majority_phylum varchar(400))'
    ...
    for one_taxon in taxon_list:
        ...
        sql = 'select seqfeature_id,phylum, count(*) as n from blastnr_blastnr A ' \
              ...
              ' where A.query_taxon_id=%s and hit_number<=%s group by seqfeature_id,phylum order by seqfeature_id, n DESC;' % (one_taxon,
                                                                                                                               n_hits)
        sql2 = 'select seqfeature_id,phylum from blastnr_blastnr A ' \
               ' inner join blastnr_blastnr_taxonomy B on A.subject_taxid=B.taxon_id  ' \
               ' where A.query_taxon_id=%s and hit_number=1;' % (one_taxon)

        cursor.execute(sql)
        data_majority = cursor.fetchall()
        cursor.execute(sql2)
        seqfeature2best_hit_phylum = manipulate_biosqldb.to_dict(cursor.fetchall())
        #print seqfeature2best_hit_phylum
        seqfeature2phylum = {}
            ...
            if row[0] not in seqfeature2phylum:
                try:
                    if row[0] == data_majority[n+1][0]:
                        ...
                        #sql = 'insert into blastnr_blastnr_majority_phylum values(%s,%s,"%s", "%s")' % ()
                        data_maj.append([one_taxon,
                                         row[0],
        ...
        df = pandas.DataFrame(data_maj, columns=["taxon_id", "seqfeature_id", "best_hit_phylum", "majority_phylum"])
        df.to_sql("blastnr_blastnr_majority_phylum", 
                                            engine, 
                                            index=False, 
                                            if_exists="append")
...
def count_less_than_n_hits(biodb, cutoff=100):
    ...
    sql = 'CREATE TABLE blastnr_count_n_blast (taxon_id INT, total INT, n_no_hits INT, n_100_hits INT, n_less_100_hits INT)'

    server.adaptor.execute(sql)

    sql = 'select query_taxon_id, seqfeature_id, count(*) as n from blastnr_blastnr group by query_taxon_id, seqfeature_id;'
    taxon2n_100 = {}
    ...
    sql = 'select taxon_id, count(*) from orthology_detail group by taxon_id'
    ...
    for taxon in taxon2n_proteins:
        ...
        sql = 'insert into blastnr_count_n_blast values (%s,%s,%s,%s, %s)' % (taxon,
                                                                               total,
...
def count_majority_phylum(biodb, 
                          hit_number=1):
    ...
    sql = 'create table if not exists blastnr_BBH_taxo_hit_number (hit_number INT, taxon_id INT, n_top_phylum INT, ' \
          ' n_non_top_phylum INT, n_euk INT, n_arch INT, n_virus INT, undefined INT)'
    ...
    for taxon_id in taxon_list:
        ...
        sql_bacteria_chlamydiae = 'select t1.query_taxon_id, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                                  ...
                                  ' and t2.phylum="%s" group by t1.query_taxon_id;' % (hit_number,
                                                                                       taxon_id,
        ...
        sql_euk = 'select t1.query_taxon_id, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                  ...
                  ' and t2.superkingdom="Eukaryota" group by t1.query_taxon_id;' % (hit_number,
                                                                                    taxon_id)

        sql_archae = 'select t1.query_taxon_id, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                  ' on t1.subject_taxid=t2.taxon_id where t1.hit_number=%s and t1.query_taxon_id=%s' \
                  ' and t2.superkingdom="Archaea" group by t1.query_taxon_id;' % (hit_number,
                                                                                  taxon_id)

        sql_viruses = 'select t1.query_taxon_id, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                  ' on t1.subject_taxid=t2.taxon_id where t1.hit_number=%s and t1.query_taxon_id=%s' \
                  ' and t2.superkingdom="Viruses" group by t1.query_taxon_id;' % (hit_number,
                                                                                  taxon_id)

        sql_undefined = 'select t1.query_taxon_id, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                  ' on t1.subject_taxid=t2.taxon_id where t1.hit_number=%s and t1.query_taxon_id=%s' \
                  ' and t2.superkingdom="-" group by t1.query_taxon_id;' % (hit_number,
                                                                            taxon_id)

        cursor.execute(sql_bacteria_chlamydiae)
        taxon2n_chlamydiae = manipulate_biosqldb.to_dict(cursor.fetchall())
        cursor.execute(sql_bacteria_non_chlamydia)
        taxon2n_non_chlamydiae = manipulate_biosqldb.to_dict(cursor.fetchall())
        cursor.execute(sql_euk)
        taxon2n_euk = manipulate_biosqldb.to_dict(cursor.fetchall())
        cursor.execute(sql_archae)
        taxon2n_arch = manipulate_biosqldb.to_dict(cursor.fetchall())
        cursor.execute(sql_viruses)
        taxon2n_virus = manipulate_biosqldb.to_dict(cursor.fetchall())
        cursor.execute(sql_undefined)
        taxon2undefined = manipulate_biosqldb.to_dict(cursor.fetchall())
        ...
        sql = 'insert into blastnr_BBH_taxo_hit_number values (%s, %s,%s,%s,%s,%s,%s,%s)' % (hit_number,
                                                                                             taxon_id,
...
def count_majority_excluding_self_species(biodb):
    ...
    sql = 'create table blastnr_BBH_taxo_hit_non_identical_%s (taxon_id INT, n_top_phylum INT, ' \
          ' n_non_top_phylum INT, n_euk INT, n_arch INT, n_virus INT, undefined INT)'
    print (sql)
    server.adaptor.execute(sql,)
    server.commit()
    ...
    for taxon_id in taxon_list:
        ...
        sql_bacteria_chlamydiae = 'select query_taxon_id,count(*) from ' \
                                  ...
                                  ' where C.phylum="%s" group by query_taxon_id;' % (taxon_id,
                                                                                       taxon2most_freq_phylum[taxon_id]
                                                                                       )
        ...
        sql_bacteria_non_chlamydia = 'select query_taxon_id,count(*) from ' \
                                  ...
                                  ' group by seqfeature_id) A )B ' \
                                  ' inner join blastnr_blastnr_taxonomy C on B.subject_taxid=C.taxon_id ' \
                                  ' where C.phylum!="%s" and C.superkingdom="Bacteria" group by query_taxon_id;' % (taxon_id,
                                                                                        taxon2most_freq_phylum[taxon_id])
        ...
        sql_euk = 'select query_taxon_id,count(*) from ' \
                                  ...
                                  ' group by seqfeature_id) A )B ' \
                                  ' inner join blastnr_blastnr_taxonomy C on B.subject_taxid=C.taxon_id ' \
                                  ' where C.superkingdom="Eukaryota" group by query_taxon_id;' % (taxon_id)
        ...
        sql_archae = 'select query_taxon_id,count(*) from ' \
                                  ...
                                  ' group by seqfeature_id) A )B ' \
                                  ' inner join blastnr_blastnr_taxonomy C on B.subject_taxid=C.taxon_id ' \
                                  ' where C.superkingdom="Archaea" group by query_taxon_id;' % (taxon_id)
        ...
        sql_viruses = 'select query_taxon_id,count(*) from ' \
                                  ...
                                  ' group by seqfeature_id) A )B ' \
                                  ' inner join blastnr_blastnr_taxonomy C on B.subject_taxid=C.taxon_id ' \
                                  ' where C.superkingdom="Viruses" group by query_taxon_id;' % (taxon_id)
        ...
        sql_undefined = 'select query_taxon_id,count(*) from ' \
                                  ...
                                  ' group by seqfeature_id) A )B ' \
                                  ' inner join blastnr_blastnr_taxonomy C on B.subject_taxid=C.taxon_id ' \
                                  ' where C.superkingdom="-" group by query_taxon_id;' % (taxon_id)
        ...
        sql = 'insert into blastnr_BBH_taxo_hit_non_identical values (%s,%s,%s,%s,%s,%s,%s)' % ( taxon_id,
                                                                                               taxon2n_chlamydiae[str(taxon_id)],
                                                                                               non_phylum,
                                                                                               euk,
                                                                                               archae,
...
def count_majority_phylum_non_identical(biodb, accession2expluded_taxon_id):
 ...
 (select * from blastnr_2017_06_29b_motile_chlamydiae where percent_identity!=100 group by seqfeature_id) A;
    ...
    sql = 'create table blastnr_BBH_taxo_hit_non_identical (taxon_id INT, n_top_phylum INT, ' \
          ' n_non_top_phylum INT, n_euk INT, n_arch INT, n_virus INT, undefined INT)'
    print (sql)
    server.adaptor.execute(sql,)
    server.commit()
...
def count_majority_phylum_consensus(biodb):
    ...
    sql = 'create table blastnr_BBH_taxo (taxon_id INT, n_top_phylum INT, ' \
          ' n_non_top_phylum INT, n_euk INT, n_arch INT, n_virus INT, undefined INT)'
    print (sql)
    server.adaptor.execute(sql,)
    server.commit()
    ...
    for taxon_id in taxon_list:
        ...
        sql_phylum = 'select t1.seqfeature_id, t2.phylum, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                                  ...
                                  ' and t2.phylum="%s" group by t1.seqfeature_id,t2.phylum order by t1.seqfeature_id,n DESC;' % (
                                                                                       taxon_id,
                                                                                       taxon2most_freq_phylum[taxon_id]
                                                                                       )
        print(sql_phylum)
        sql_superkingdom = 'select t1.seqfeature_id, t2.superkingdom, count(*) as n from blastnr_blastnr t1 inner join blastnr_blastnr_taxonomy t2 ' \
                                  ...
                                  ' and t2.phylum!="%s" group by t1.query_taxon_id;' % (taxon_id,
                                                                                        taxon2most_freq_phylum[taxon_id])
        ...
        sql = 'insert into blastnr_blast_hits_taxonomy_overview values (%s,%s,%s,%s,%s,%s,%s)' % (taxon_id,
                                                                                             n_chlam,
                                                                                             n_non_chlamydiae,
                                                                                             n_euk,
                                                                                             n_arch,
